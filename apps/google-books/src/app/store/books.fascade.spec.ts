...
describe('bookFascade', () => {
  ...
  it('should call action with seathterm', () => {
    ...
    expect(spy).toBeCalledWith(fromAction.loadBooks({ searchTerm: 'java' }));
  });
  it('should be add to cart action', () => {
    const spy = spyOn(store, 'dispatch');
    facade.addToCart({ book: books[0] });
    expect(spy).toBeCalledWith(cartAction.addBook({ book: books[0] }));
  });
  ...
  it('should remove from cart ', () => {
    ...
    expect(spy).toBeCalledWith(cartAction.deleteUser({ id: '0BSOg0oHhZ0C' }));
  });
  ...
  it('should  add user ', () => {
    ...
    const spy = spyOn(store, 'dispatch');
    facade.addUser({ user });
    expect(spy).toBeCalledWith(fromAction.addUser({ user }));
  });
});
